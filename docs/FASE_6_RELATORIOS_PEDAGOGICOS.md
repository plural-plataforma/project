...
- **S4 — Revisão/edição/status (backend):** `AtualizarSecaoAsync` (`PATCH /api/Relatorio/{id}/secoes`) edita `TextoEditado` de uma seção (sem campo de notas manuais, removido numa revisão posterior — ver seção "Versão final" abaixo), bloqueado se `Status == Finalizado` ou `Status == RevisaoFinal`. `SolicitarRevisaoFinalAsync` (`POST /api/Relatorio/{id}/finalizar`) exige que já tenha sido gerado e recebe o formato escolhido pela professora (`RelatorioFormatoFinal`: Tópicos ou Texto corrido); finaliza direto se for Tópicos sem nenhuma seção editada, senão vai para `Status.RevisaoFinal` e enfileira a revisão por IA em background (`ProcessarRevisaoFinalAsync`). `AceitarRevisaoFinalAsync`/`DescartarRevisaoFinalAsync` (`POST /api/Relatorio/{id}/revisao-final/aceitar|descartar`) decidem o resultado da revisão. `ReabrirAsync` (`POST /api/Relatorio/{id}/reabrir`) volta pra Rascunho **sobrescrevendo o mesmo registro** (sem versionamento, decisão da cliente), limpando também o estado de finalização (`FormatoFinal`/`TextoFinal`/`TextoFinalGeradoEm`/`TextoRevisado`). `BuscarPorIdAsync` (`GET /api/Relatorio/{id}`) retorna o relatório com as seções.
...
- `src/pages/relatorio/RelatorioDetailPage.tsx` — tela de revisão pedida na S4: as 14 seções em cards editáveis (`TextoEditado`, sem campo de notas manuais e sem botão de reescrever seção com IA — ambos removidos numa revisão posterior), badge de "informação insuficiente" por seção, bloqueio de edição quando `Finalizado` ou `RevisaoFinal`, botões Finalizar/Reabrir/Gerar novamente/Duplicar. Ao finalizar, a professora escolhe o formato (Tópicos ou Texto corrido); se houver seção editada ou o formato for Texto corrido, a tela mostra a revisão da IA em `RelatorioRevisaoFinal.tsx` para aceitar ou descartar antes de congelar. Rota `/relatorios-pedagogicos/:id`.
...
- `report_sections` (`RelatorioSecao`): `Id`, `RelatorioId`, `SecaoChave` (contextualizacao, potencialidades, comunicacao, cognicao, academico, interacao, autonomia, motor_sensorial, barreiras, estrategias, evolucao, necessidades, encaminhamentos, conclusao — 14 chaves + identificação tratada à parte, ver 4.3), `TextoGerado`, `TextoEditado`, `TextoRevisado`, `GeradoEm`, `EditadoEm` (sem `NotasManuais` — campo removido numa revisão posterior). `Relatorio` também ganhou `FormatoFinal`, `TextoFinal` e `TextoFinalGeradoEm` pra suportar a versão final (ver 4.4).
...
- Tela de revisão com as 15 seções, texto gerado editável por seção (`TextoEditado`) — sem campo de notas manuais e sem botão de reescrever seção com IA (ambos removidos numa revisão posterior).
- Sem botão de regeneração por seção (decisão da seção 3).
- Ao finalizar, a professora escolhe o formato do documento (Tópicos ou Texto corrido). Se não houver seção editada e o formato for Tópicos, finaliza direto; caso contrário, a IA revisa o texto em segundo plano e a professora aceita ou descarta a proposta antes de o relatório congelar.
- Fluxo de status: Rascunho → Gerando → (Rascunho, editável) → RevisãoFinal (quando aplicável) → Finalizado, com ação "Reabrir para edição" que volta pra Rascunho e permite sobrescrever.
...
| **S4** | Tela de revisão/edição | 15 seções editáveis (sem notas manuais); finalização com escolha de formato e revisão final por IA (aceitar/descartar); fluxo Rascunho → RevisãoFinal → Finalizado → Reabrir (sobrescreve). |
