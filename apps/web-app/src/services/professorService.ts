...
export const atualizarProfessor = async (professorData: Professor): Promise<ProfessorResponse> => {
  const payload = { ...professorData, escolas: [] }
  const response = await api.patch<ProfessorResponse>('/Professor/atualizar/', payload)
  return response.data
}
...
export interface CadastroPendencia {
  key: 'nomeCompleto' | 'sexo' | 'cep' | 'estado' | 'telefone' | 'escola'
  label: string
}
...
export const getCadastroPendencias = (
  ...
  if (!hasValue(professor?.sexo)) {
    pendencias.push({ key: 'sexo', label: 'Sexo' })
  }
