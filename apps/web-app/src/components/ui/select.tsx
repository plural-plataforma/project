...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-50 min-w-[8rem] overflow-hidden rounded-xl border border-border bg-card shadow-elevated',
    ...
    >
      <SelectPrimitive.Viewport
        className={cn(
          'p-1.5 max-h-72 overflow-y-auto overscroll-contain',
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn('py-1.5 pl-8 pr-3 text-xs font-semibold text-muted-foreground', className)}
    {...props}
  />
))
